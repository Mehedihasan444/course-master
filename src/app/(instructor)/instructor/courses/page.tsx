...
export default async function InstructorCoursesPage({
  ...
  const courses = await getInstructorCourses(user!._id.toString(), params);

  const publishedCount = courses.filter((c: { isPublished: boolean }) => c.isPublished).length;
  const draftCount = courses.filter((c: { isPublished: boolean }) => !c.isPublished).length;
